...
public class UsersQueueExtension implements
  ...
  public void beforeEach(ExtensionContext context)  {
    ...
    List<Map<UserType, UserJson>> usersList = new ArrayList<>();
  ...
  public void afterTestExecution(ExtensionContext context) {
    final String testId = getTestId(context);
    List<Map<UserType, UserJson>> users = (List<Map<UserType, niffler.model.UserJson>>) context.getStore(USER_EXTENSION_NAMESPACE)
            .get(testId);
  ...
  public UserJson resolveParameter(ParameterContext parameterContext,
    ...
    List<Map<UserType, UserJson>> users = (ArrayList<Map<UserType, UserJson>>) extensionContext.getStore(USER_EXTENSION_NAMESPACE)
            .get(testId);
    ArrayList<Object> arr = new ArrayList<>();
    for (Map<UserType, UserJson> userMap : users) {
      arr.add(userMap.entrySet().iterator().next().getValue());
    }
    UserJson user = (UserJson) arr.get(parameterContext.getIndex());
    return user;
  }
